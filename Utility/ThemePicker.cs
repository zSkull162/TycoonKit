...
public class ThemeOptionsEditor : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        EditorGUILayout.BeginVertical(EditorStyles.helpBox);
        InspectorUtils.Description("<color=#b4b4b4>This is important. It store your currently selected theme, so that it won't get reset when scripts compile, or when you exit Unity.</color> <color=#8c8c8c><i>This theme picker also couldn't do anything without this, as it writes your selected theme to this ScriptableObject.</i></color>");
        ...
        if (script.useCustom)
        {
            ...
            InspectorUtils.Description("To save your custom theme, click the \"Save theme\" button. To load a custom theme, place a Theme Container from the assets into the slot on the left, and click \"Load theme\"");
            EditorGUILayout.Space(2);
            ...
            if (GUILayout.Button("Save theme"))
            {
                ...
                {
                    Logger.Log("Theme Picker", "Custom theme not currently applied!", LogColor.Red, false);
                    GUIUtility.ExitGUI();
                    return;
                }
                ...
                Logger.Log("<b>Theme Picker</b>", $"<b>Theme saved to</b> \"{path}\"", LogColor.Lime, false);
                GUIUtility.ExitGUI();
            }
            if (GUILayout.Button("Load theme"))
            {
                ...
                {
                    Logger.Log("Theme Picker", "Theme Container not found!", LogColor.Red, false);
                    GUIUtility.ExitGUI();
                    return;
                }
                ...
                EditorUtility.SetDirty(script.themeContainer);
                if (!script.isThemeCustom) script.isThemeCustom = true;
                Logger.Log("Theme Picker", $"Custom theme loaded!", LogColor.LightBlue, true);
